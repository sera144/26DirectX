...
Player players[2] = {
    { -0.5f, 0.0f, 0 }, // Player 1 (WASD)
    {  0.5f, 0.0f, 1 }  // Player 2 (Arrow)
};
...
void Update()
{
    ...
    // Player 1 (WASD)
    if (GetAsyncKeyState('A') & 0x8000) players[0].posX -= speed;
    if (GetAsyncKeyState('D') & 0x8000) players[0].posX += speed;
    if (GetAsyncKeyState('W') & 0x8000) players[0].posY += speed;
    if (GetAsyncKeyState('S') & 0x8000) players[0].posY -= speed;

    // Player 2 (Arrow keys)
    if (GetAsyncKeyState(VK_LEFT) & 0x8000)  players[1].posX -= speed;
    if (GetAsyncKeyState(VK_RIGHT) & 0x8000) players[1].posX += speed;
    if (GetAsyncKeyState(VK_UP) & 0x8000)    players[1].posY += speed;
    if (GetAsyncKeyState(VK_DOWN) & 0x8000)  players[1].posY -= speed;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow)
{
    ...
    HWND hWnd = CreateWindowW(L"DX11Class", L"2 Player Triangles",
    ...
    CUSTOMVertex baseVertices[] = {
        { 0.0f,  0.3f, 0.0f, 1,0,0,1 },
        { 0.3f, -0.3f, 0.0f, 0,1,0,1 },
        {-0.3f, -0.3f, 0.0f, 0,0,1,1 },

    };

    // Vertex buffer (2 players)
    ID3D11Buffer* pVBuffer;

    D3D11_BUFFER_DESC bd = {};
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof(CUSTOMVertex) * 6; //2 player x 3 vertices
    bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;

    g_pd3dDevice->CreateBuffer(&bd, NULL, &pVBuffer);
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        else
        {
            ...
            for (int p = 0; p < 2; p++)
            {
                ...
                {
                    int index = p * 3 + i;
                    moved[index] = baseVertices[i];
                    moved[index].x += players[p].posX;
                    moved[index].y += players[p].posY;
                }
            }
